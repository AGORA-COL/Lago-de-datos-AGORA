...
def read_and_validate_file_from_hdfs(spark, input_file, expected_columns, error_handler, is_first_file):
    ...
    if is_first_file:
        ...
        rdd = rdd.filter(lambda ln: ln != header_line)
    else:
        header_line = None

    rdd_split = rdd.map(lambda ln: ln.split(','))

    valid_lines_rdd = rdd_split.filter(lambda ln: len(ln) == expected_columns)
    invalid_lines_rdd = rdd_split.filter(lambda ln: len(ln) != expected_columns)
    for line in invalid_lines_rdd.collect():
        # Cuidado!, si las líneas de error son muchas podemos tener problemas de memoria
        error_handler.handle_error(line)

    return valid_lines_rdd, valid_lines_rdd.count(), header_line


def write_to_parquet(spark, valid_lines_rdd_list, output_path, columns):
    """
    Escribe las líneas válidas en un archivo Parquet usando Spark.
    """
    dataframes = [spark.createDataFrame(rdd.map(lambda line: Row(*line)), columns) for rdd in valid_lines_rdd_list]
    ...
    for source in config['sources']:
        if source['process']:  # Verifica si la fuente está activada para procesar
            ...
            for i, input_file in enumerate(source['input_files']):
                ...
                valid_lines_rdd, total_lines, file_header = read_and_validate_file_from_hdfs(spark, input_path,
                                                                                             len(source[
                                                                                                     'input_columns']),
                                                                                             error_handler,
                                                                                             is_first_file=(i == 0))
                if i == 0:
                    ...
                    if header_line and file_header and header_line != file_header:
                        raise ValueError(
                            f"El encabezado del archivo {input_file} no coincide con el del primer archivo.")
                total_records += total_lines
                all_valid_lines.append(valid_lines_rdd)
